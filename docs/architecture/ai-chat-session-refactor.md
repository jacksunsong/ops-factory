...
| HTTP 状态 | 含义 | 前端处理 |
| --- | --- | --- |
| 200/202 | cancel 已提交或 goosed 认为幂等成功 | 进入 `cancelling`，等待 events 确认或短暂后显示 cancelled |
| 400 | request id 非法或不可取消 | 展示明确错误 |
| 401/403 | 鉴权失败 | 提示登录/权限 |
| 404 | session/request 不存在 | 如果本地已无 active request，可视为已结束；否则展示错误 |
| 502/503/504 | cancel 无法送达 goosed | 提示取消未确认，可重试取消或等待 |

cancel 要求：

- 用户点击停止时必须携带当前 `request_id`。
- cancel 请求失败不能伪装成已取消。
- 如果 cancel HTTP 成功但没有收到后续取消事件，前端应显示“取消已提交，等待确认”一类状态，而不是立刻删除上下文。
...
- `web-app/src/app/platform/chat/useChat.ts`
  ...
  - 发送流程调整为：
    ...
    5. finish/error/cancel 后结束该 request。
  - stop 流程调整为：
    - 新路径调用 `cancelSessionReply(sessionId, requestId)`。
    - 只关闭本地 listener，不用它表达后端取消。
