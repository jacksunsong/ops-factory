...
以 `/agents/{agentId}/sessions/{sessionId}/reply` + `/events` 为例的完整链路：

```
1. Client GET /agents/{agentId}/sessions/{sessionId}/events
   Client POST /agents/{agentId}/sessions/{sessionId}/reply
...
5. ReplyController 分离提交和订阅
   └── POST → goosed:{port}/sessions/{sessionId}/reply
   └── GET  → goosed:{port}/sessions/{sessionId}/events
   └── 透传 SSE 事件给客户端，客户端断开不自动 cancel
```
...
### 4.3 Session Events Proxy — SSE 流转发

SSE (Server-Sent Events) 是 goosed 返回聊天回复的通信方式。Gateway 的 session events proxy 负责：

- 透传 goosed SSE frame，包括 `id:`、`data:` 和 heartbeat comment
- 转发 `Last-Event-ID`，支持浏览器重连补事件
- 客户端断开只结束本次订阅，不自动取消 goosed active request
- 用户显式停止时才通过 `/sessions/{sessionId}/cancel` 取消 request
- Gateway 自身错误通过结构化 JSON 或 SSE `Error` 如实呈现
...
### 5.4 Session Events 阶段 — 超时策略

旧 `/reply` 链路曾使用三层超时保护。新 session events 链路不再使用浏览器请求时长作为取消信号：

| 层级 | 名称 | 默认值 | 触发条件 | 处理方式 |
|------|------|--------|----------|----------|
| 层级 | 名称 | 触发条件 | 处理方式 |
|------|------|----------|----------|
| Process | watchdog | goosed 进程退出或健康检查失败 | 回收实例并按退避重启 |
| Events | reconnect | 浏览器网络断开或页面刷新 | 用 `Last-Event-ID` 重连，不 cancel request |
| Product | max duration | 产品定义的单次任务硬上限 | 调用 `/sessions/{sessionId}/cancel`，并向前端展示明确原因 |

旧 first-byte/idle/max-duration 的数值不能再作为前端 fetch timeout 或隐式 stop 依据。LLM 慢、tool 慢、网络断开和用户取消必须在事件与错误契约中区分。
...
当 Gateway 健康检查或明确的不可恢复错误判定 goosed 不可用时：
